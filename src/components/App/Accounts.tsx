...
import { fetchAccounts, generateToTp } from "../../lib/totp.ts";
...
export function Accounts({
  ...
  loginCode: string;
  updateAccounts: ToTpAccount[];
}) {
  const [search, setSearch] = useState<string>();
  const [accounts, setAccounts] = useState<ToTpAccount[]>([]);
  const [selectedAccount, selectAccount] = useState<ToTpAccount | null>(null);
  ...
  const loadAccounts = async () => {
    ...
    const data = await fetchAccounts(loginCode);
  ...
  useEffect(() => {
    if (updateAccounts.length > 0) setAccounts(updateAccounts);
  ...
  return (
    <>
      <ToTpModal
        ...
        onClose={(b) => (b == false ? selectAccount(null) : null)}
      />
      ...
      <div className="w-full max-w-md flex flex-col gap-6 p-4">
        {accounts.length >= 1 ? (
                ...
                <div className="flex items-center gap-4">
                  <img
                    ...
                    src={`https://api.iconify.design/${
                      stringToIcon(a.Icon ?? "ic:baseline-fingerprint")?.prefix
                    }/${
          ...
          <div>
            ...
            <span className={"flex items-center justify-center"}>
              Use the <Plus className={"mr-1 ml-1 size-6"} color={"#808080"} />{" "}
              symbol to create your first account.
            </span>
          </div>
        )}
      </div>
